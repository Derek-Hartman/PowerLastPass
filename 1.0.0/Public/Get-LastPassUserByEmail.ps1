<#
.Synopsis
    Retrieve specfic lastpass user details by email

.EXAMPLE 
    Get-LastPassUserByEmail -CID "####" -ProvisioningHash "####" -Email ""
...
Function Get-LastPassUserByEmail {
[CmdletBinding()]
    param(
        ...
        [Parameter(Mandatory = $True,
            ValueFromPipeline = $True,
            HelpMessage = "Enter the users email.")]
        [string[]]$Email
    )

    $Uri = "https://lastpass.com/enterpriseapi.php"

    $UBody = @{"username" = "$Email"}
    $Body = @{"cid" = "$CID"}
    ...
    If ([string]::IsNullOrEmpty($Response.Users)) {
        $Output = $Response
    }
    Else {
